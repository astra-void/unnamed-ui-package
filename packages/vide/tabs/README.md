...
A tab list and the one panel that is mounted at a time.

```bash
npx lattice-ui add tabs --framework vide
```
...
The behavior lives in `@lattice-ui/core-tabs`, which imports no framework at all and is the same code the React layer runs. This package is the Vide half: it builds that core on Vide's reactivity and renders what the core describes.
