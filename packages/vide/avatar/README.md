...
An image with a fallback that waits before it appears.

```bash
npx lattice-ui add avatar --framework vide
```
...
The behavior lives in `@lattice-ui/core-avatar`, which imports no framework at all and is the same code the React layer runs. This package is the Vide half: it builds that core on Vide's reactivity and renders what the core describes.
