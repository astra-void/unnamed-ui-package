...
A scrolling viewport with a scrollbar the primitive measures and you paint.

```bash
npx lattice-ui add scroll-area --framework vide
```
...
The behavior lives in `@lattice-ui/core-scroll-area`, which imports no framework at all and is the same code the React layer runs. This package is the Vide half: it builds that core on Vide's reactivity and renders what the core describes.
