...
A toast queue with a visible window and a timer per entry.

```bash
npx lattice-ui add toast --framework vide
```

## Parts

`Toast.Provider`, `Toast.Viewport`, `Toast.Root`, `Toast.Title`, `Toast.Description`, `Toast.Action`, `Toast.Close`

## What this package is

The behavior lives in `@lattice-ui/core-toast`, which imports no framework at all and is the same code the React layer runs. This package is the Vide half: it builds that core on Vide's reactivity and renders what the core describes.
