...
```bash
npx lattice-ui add toggle-group --framework vide
```
...
The behavior lives in `@lattice-ui/core-toggle-group`, which imports no framework at all and is the same code the React layer runs. This package is the Vide half: it builds that core on Vide's reactivity and renders what the core describes.
