...
// An unstyled menu has nothing to animate, so there is no default entrance recipe. Presence
...
export function ContextMenuContent(props: ContextMenuContentProps) {
  ...
  const popper = createPopper(rx, {
    getAnchor: core.getVirtualAnchor,
  ...
  function renderContent() {
    ...
    if (props.asChild === true) {
      ...
      if (child === undefined) {
        error("[ContextMenuContent] `asChild` requires a child instance.");
      }
  ...
  return (
    ...
    >
      ...
      {/* A 1x1 virtual anchor at the pointer, so the shared popper places and flips the menu against
          the viewport exactly as it does for a real anchor. */}
      <frame
        action={(instance: Frame) => core.setVirtualAnchor(instance)}
        BackgroundTransparency={1}
        BorderSizePixel={0}
        Position={() => {
          const anchor = core.anchorPosition();
          return UDim2.fromOffset(anchor.X, anchor.Y);
        }}
