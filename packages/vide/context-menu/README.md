...
A menu anchored to the pointer rather than to a trigger.

```bash
npx lattice-ui add context-menu --framework vide
```

## Parts

`ContextMenu.Root`, `ContextMenu.Trigger`, `ContextMenu.Portal`, `ContextMenu.Content`, `ContextMenu.Item`, `ContextMenu.Group`, `ContextMenu.Label`, `ContextMenu.Separator`

## What this package is

The behavior lives in `@lattice-ui/core-context-menu`, which imports no framework at all and is the same code the React layer runs. This package is the Vide half: it builds that core on Vide's reactivity and renders what the core describes.
