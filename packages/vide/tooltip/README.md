...
A tooltip with a group delay and a skip window.

```bash
npx lattice-ui add tooltip --framework vide
```

## Parts

`Tooltip.Provider`, `Tooltip.Root`, `Tooltip.Trigger`, `Tooltip.Portal`, `Tooltip.Content`

## What this package is

The behavior lives in `@lattice-ui/core-tooltip`, which imports no framework at all and is the same code the React layer runs. This package is the Vide half: it builds that core on Vide's reactivity and renders what the core describes.
