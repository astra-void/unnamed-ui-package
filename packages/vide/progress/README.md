...
A determinate or indeterminate progress bar, and a spinner.

```bash
npx lattice-ui add progress --framework vide
```

## Parts

`Progress.Root`, `Progress.Indicator`, `Progress.Spinner`

## What this package is

The behavior lives in `@lattice-ui/core-progress`, which imports no framework at all and is the same code the React layer runs. This package is the Vide half: it builds that core on Vide's reactivity and renders what the core describes.
